...
export async function POST(req: NextRequest) {
  ...
  // 1. Upsert company
  await db.insert(companies).values({ name: data.company_name }).onConflictDoNothing()
  const [company] = await db.select({ id: companies.id }).from(companies).where(eq(companies.name, data.company_name)).limit(1)
  ...
  async function upsertTags(
    ...
    for (const name of names) {
      await db.insert(lookupTable).values({ name }).onConflictDoNothing()
      const [row] = await db.select({ id: lookupTable.id }).from(lookupTable).where(eq(lookupTable.name, name)).limit(1)
      await junctionInsert(row.id)
    }
  }
